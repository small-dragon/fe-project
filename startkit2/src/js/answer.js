var A = (function(){
	var NO_LOGIN_DEBUG = false,    //PC测试用 
        ...
        weixinShare = function(title, desc, logoUrl, link) {
            var shareData = {
                title: title || "中国邮政开办120周年，疯狂邮戳盖盖盖，看看你有多猴赛雷！",
                desc: desc || "猴年盖猴票，让你猴赛雷，试一试你30秒能盖多少张？开始游戏吧~",
                link: link || location.href.split('#')[0],
                imgUrl: logoUrl || (baseHref + "image/logo/big-logo.jpg"),
		...
		login = function(){
			var dtd = $.Deferred();
			getJsonP(serverurl + "/index.php/Activity/Dadishu/isLogin", '',  'isLoginCallback')
			.then(function(data){
				...
				if(data.status == 1){
					...
					var appId = data.data && data.data.appid || "wxac7d69a69e29b843",
                    	...
                    	weixinUrl = serverurl + "/index.php/Activity/Dadishu/go2login?jump=" + currentUrl;
        ...
        buildGame = function(){
        	var data,
                isDue = Date.now() < new Date('2016/10/1 12:00')?true:false; //10.1:12:00准时开始活动
        ...
        init = function(){
        		...
        		//noAp = location.href.match(/[?&]noap=([^&#*])/i);
    		//noAp = noAp && noAp[1];
